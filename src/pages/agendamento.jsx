...
import { api8080WToken, api8080WTokenEmpresas, api8080WTokenUsuarios } from "../api/api";
...
function Agendamento() {
  ...
  useEffect(() => {
    ...
    const getEndereco = async () => {
      try {
        const response = await api8080WTokenUsuarios.get(`/usuarios/endereco/${idUsuario}`);
    ...
    const getServico = async () => {
      try {
        const response = await api8080WTokenEmpresas.get(`/empresas/servicos/${idServico}`);
  ...
  function agendar(id) {
    ...
    if (endereco) {
      ...
      api8080WTokenEmpresas.post("/empresas/pagamentos/criar-pagamento", {
        cpf: cpf,
        nome: usuario?.nome,
        email: email,
        valor: 0.01,
